...
test("user can draft a work journal entry with Copy Paste", async ({ page }) => {
  ...
  await page
    .getByPlaceholder(t.notesPlaceholder)
    .fill(rawNotes);
  await page.getByRole("button", { name: t.aiWriting }).click();

  await expect(
    page.getByRole("button", { name: copyPaste.open })
  ).toBeVisible();
  await page.getByRole("button", { name: copyPaste.open }).click();
  ...
  const finalText = page.getByLabel(t.finalText);
  ...
  await expect(savedEntry.getByText(rawNotes)).toBeVisible();
});
...
test("integrated AI drafting remains available when an API key is configured", async ({
  ...
  await expect(
    page.getByRole("button", { name: t.generateProfessionalDraft })
  ).toBeVisible();
});
