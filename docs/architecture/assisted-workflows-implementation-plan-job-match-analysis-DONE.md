...
For the Job Match Analysis pilot preview, show:
...
| --- | --- | --- | --- | --- |
| job match analysis scoring | Planned/manual result editing not yet designed | Pilot | Existing via API key/mock provider-aware factories | First structured Copy Paste pilot. |
| Job match scoring | Planned/manual result editing not yet designed | Planned | Existing via API key/mock provider-aware factories | Should follow job match analysis once the pilot is proven. |
...
### Phase 2: Job Match Analysis Copy Paste backend
...
```txt
src/app/api/job-match-analyses/[id]/score/copy-paste/prepare/route.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/preview/route.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/apply/route.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/*/responses.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/*/validation.ts
```

Use cases:

```txt
src/modules/job-match-analysis/application/use-cases/prepare-job-match-score-copy-paste.use-case.ts
src/modules/job-match-analysis/application/use-cases/preview-job-match-score-copy-paste.use-case.ts
src/modules/job-match-analysis/application/use-cases/apply-job-match-score-copy-paste.use-case.ts
```

Prompt work:

```txt
src/modules/job-match-analysis/infrastructure/services/job-match-scoring-prompts.ts
docs/prompts/analisis-oferta/prompt.md
```

Tasks:

1. Inspect current `ScoreJobMatchAnalysisUseCase`, `UpdateJobMatchAnalysisAIResultUseCase`, `JobMatchScoringAIService`, and Gemini/mock result shape.
2. Extract or reuse the scoring result schema so Integrated and Copy Paste apply the same domain result.
3. Add `buildJobMatchScoringCopyPastePrompt(input)` using the same semantic criteria as the integrated prompt.
4. `prepare` route:
   - use `getAuthenticatedRequestContext()`
   - validate route/body input before binding
   - bind `jobMatchAnalysisModule`
...
5. `preview` route:
   - parse raw response
   - validate envelope
   - validate Job Match scoring result shape
   - return parsed result and summarized preview
6. `apply` route:
   - revalidate parsed result
   - bind module
   - call common persistence path, ideally `updateJobMatchAnalysisAIResult.execute(...)`
   - use `aiModel: "external-chat"`
   - record observability with `assistanceMode`, `workflowId`, `schemaVersion`
   - return `ScoreJobMatchAnalysisResponse`
7. Add use-case tests for prepare, preview, and apply.
8. Add route validation/response tests where useful.
9. Update prompt documentation under `docs/prompts/analisis-oferta/prompt.md`.

Verification:

- `npm run test:backend -- job-match-analysis`
- `npm run ddd:check`
- `npm run build`

### Phase 3: shared frontend Copy Paste wizard and UI reuse
...
src/components/shared/copy-paste-workflow-preview-summary.tsx
```

Feature-specific composition wrapper, only if Job Match needs custom preview fields or custom apply orchestration:
...
1. Reuse `CopyPasteWorkflowTriggerButton` for every entry point. Do not duplicate icon, accessibility, responsive hidden-label behavior, or base styling in feature views.
2. Use one shared structured Copy Paste modal for JSON-envelope workflows. Do not create a second full feature-local modal by copying `CVScoreCopyPasteModal`. The shared modal should receive feature-specific functions/configuration through props:
   - `title`
   - `intro`
   - `prepare`
   - `preview`
...
3. If a workflow needs unique preview content, use composition: pass a feature-specific preview component into the shared modal. Do not fork the whole modal for a small preview difference.
4. Keep feature wrappers thin. `CVScoreCopyPasteModal` and `JobMatchScoreCopyPasteModal` should mainly wire typed API callbacks, translations, and preview composition into the shared modal.
5. Refactor the existing CV Analysis Copy Paste UI while implementing Job Match so both the old workflow and the new workflow use the same shared modal structure. The result should not be one generic modal for Job Match and a separate bespoke modal for CV.
6. Feature wrappers may live under `src/features/<feature>/components/`, but reusable button/modal/step components must live under `src/components/shared/`.

Tasks:

1. Add frontend API client functions for Job Match Analysis Copy Paste endpoints.
2. Add a visible local action in the Job Match Analysis scoring UI:
...
### Phase 4: shared UI extraction follow-up

Goal: verify that CV Analysis and Job Match both use the same shared structured Copy Paste modal shape, and remove any remaining duplicated Copy Paste UI.

Tasks:

1. Compare the CV Analysis and Job Match Copy Paste wrappers.
2. Confirm identical step/state/copy/paste/error/apply behavior lives in `src/components/shared/`.
3. Confirm feature folders contain only feature-specific API clients, preview renderers, workflow labels, and typed wrapper wiring.
4. If any duplicated modal logic remains in `CVScoreCopyPasteModal`, move it into the shared modal before finishing this phase.
5. Verify both workflows still pass E2E after extraction.

Verification:

- `npm run test:e2e:local -- cv-analysis-copy-paste.spec.ts`
- `npm run test:e2e:local -- job-match-analysis-copy-paste.spec.ts`
- `npm run build`
...
## Ultra-detailed Job Match Analysis pilot implementation checklist
...
Do not implement CV Analysis changes, CV editor, templates, Work Journal, Feedback Notes, global settings, Ollama, or app subscriptions in this plan.
...
- User has a job match analysis draft/extraction screen.
...
1. User enters the job description and optional job URL.
2. User clicks "Analyze with external AI".
3. App opens a large Copy Paste modal.
4. App prepares a prompt from backend CV extraction data plus the provided job description/URL.
5. Modal shows a non-blocking privacy warning.
6. User copies the prompt.
7. User pastes the prompt into an external chat app.
8. External chat returns JSON with the required envelope.
9. User pastes the response into the modal.
10. App validates response through backend `preview`.
11. Modal shows a summarized read-only preview.
12. User clicks "Apply analysis".
13. Backend applies the result with `aiModel: "external-chat"` and persists the job snapshot.
14. App closes modal and shows the normal analysis result screen.
...
```ts
interface PrepareJobMatchAnalysisCopyPasteResponse {
  workflowId: "job_match_analysis.score";
  schemaVersion: "1";
  prompt: string;
  expectedResponse: {
...
```ts
interface PreviewJobMatchAnalysisCopyPasteRequest {
  rawResponse: string;
}
```
...
```ts
interface PreviewJobMatchAnalysisCopyPasteResponse {
...
```ts
interface ApplyJobMatchAnalysisCopyPasteRequest {
  parsedResult: JobMatchAnalysisCopyPasteResult;
...
Use the existing `ScoreJobMatchAnalysisResponse` shape from:

```txt
src/app/api/job-match-analyses/responses.ts
```
...
The shape of `result` must match the existing Job Match scoring AI result as closely as possible. Do not invent a second scoring domain shape unless the current shape cannot be reused.

### Step 1: inspect current Job Match scoring implementation

Files to read first:

```txt
src/modules/job-match-analysis/application/use-cases/score-job-match-analysis.use-case.ts
src/modules/job-match-analysis/application/use-cases/update-job-match-analysis-ai-result.use-case.ts
src/modules/job-match-analysis/domain/repositories/job-match-scoring-ai.service.ts
src/modules/job-match-analysis/infrastructure/services/gemini-job-match-scoring-ai.service.ts
src/modules/job-match-analysis/infrastructure/services/mock-job-match-scoring-ai.service.ts
src/modules/job-match-analysis/infrastructure/services/job-match-scoring-prompts.ts
src/app/api/job-match-analyses/[id]/score/route.ts
src/app/api/job-match-analyses/validation.ts
src/app/api/job-match-analyses/responses.ts
docs/prompts/analisis-oferta/prompt.md
```
...
1. What exact object does `JobMatchScoringAIService` return?
2. What exact input does `UpdateJobMatchAnalysisAIResultUseCase` need?
3. Where is best CV text selected today?
4. Where are `jobDescription` and `jobUrl` normalized today?
...
4. Keep these utilities free of Job Match Analysis imports.
...
### Step 3: Job Match scoring result validation

Add a Job Match-analysis-specific validator for the pasted result.
...
```txt
src/modules/job-match-analysis/application/services/job-match-score-copy-paste-result.validator.ts
```

Rules:

1. Validate all required fields expected by current Job Match scoring persistence.
...
```ts
buildJobMatchScoringCopyPastePrompt(input)
```
...
6. Preserve/output user-facing analysis language according to existing Job Match scoring behavior.
...
1. Receive `{ id, userId, jobDescription, jobUrl? }`.
...
5. Build prompt through `buildJobMatchScoringCopyPastePrompt`.
...
3. Parse JSON.
4. Validate envelope.
5. Validate Job Match scoring result.
...
4. Call the common persistence path:
   - prefer `UpdateJobMatchAnalysisAIResultUseCase`
   - or shared domain/application method used by `ScoreJobMatchAnalysisUseCase`
5. Set model/source:
   - `aiModel: "external-chat"`
...
```txt
src/modules/job-match-analysis/application/use-cases/prepare-job-match-score-copy-paste.use-case.test.ts
src/modules/job-match-analysis/application/use-cases/preview-job-match-score-copy-paste.use-case.test.ts
src/modules/job-match-analysis/application/use-cases/apply-job-match-score-copy-paste.use-case.test.ts
```
...
src/modules/job-match-analysis/index.ts
...
```txt
src/app/api/job-match-analyses/[id]/score/copy-paste/prepare/route.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/prepare/responses.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/prepare/validation.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/preview/route.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/preview/responses.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/preview/validation.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/apply/route.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/apply/responses.ts
src/app/api/job-match-analyses/[id]/score/copy-paste/apply/validation.ts
```

If duplication is high, share route-local helpers under:

```txt
src/app/api/job-match-analyses/[id]/score/copy-paste/_shared/
```
...
```txt
src/features/job-match-analysis/api/job-match-analysis-copy-paste-api.ts
```

or add to:

```txt
src/features/job-match-analysis/api/job-match-analysis-api.ts
```

Functions:

```ts
prepareJobMatchAnalysisCopyPaste(id, input)
previewJobMatchAnalysisCopyPaste(id, input)
applyJobMatchAnalysisCopyPaste(id, input)
```
...
Use a shared structured Copy Paste modal for the copy/paste/review workflow. Use a feature-specific wrapper only for Job Match API functions, labels, job-description/job-URL handling, and preview composition.

The existing `CVScoreCopyPasteModal` must be updated as part of this work so CV Analysis and Job Match follow the same structure:

```txt
shared modal owns workflow mechanics
feature wrapper owns typed callbacks, labels, and preview composition
```
...
Shared modal responsibilities:

- render modal frame, header, close button, and step indicator
- manage `copy -> paste -> review` state
- call `prepare` on open
- handle loading/error states
- own prompt copying
- own pasted response textarea
- own backend preview call from pasted text
- preserve invalid pasted text after errors
- own correction-instructions copy action
- own apply button placement and loading state
- call `onApplied` and close/reset after success

Feature wrapper responsibilities:

- provide typed `prepare`, `preview`, and `apply` callbacks
- provide translated title/intro/labels
- provide workflow-specific correction instructions
- provide `renderPreview(previewData)`
- provide `getApplyLabel(previewData)`
- map `onApplied` to the feature's cache/update callback
...
<CopyPasteWorkflowModal
  ...
  title={t("title")}
  intro={t("intro")}
  prepare={() => prepareJobMatchAnalysisCopyPaste(analysisId, input)}
  preview={(rawResponse) =>
    previewJobMatchAnalysisCopyPaste(analysisId, { rawResponse })
  }
  apply={(parsedResult) =>
    applyJobMatchAnalysisCopyPaste(analysisId, { parsedResult })
  }
  renderPreview={(preview) => (
    <JobMatchScoreCopyPastePreview preview={preview} />
  )}
  getApplyLabel={(preview) =>
    preview.preview.willReplaceExistingResult
      ? t("replaceAnalysis")
      : t("applyAnalysis")
  }
  getCorrectionInstructions={() => correctionInstructions}
  onApplied={onApplied}
/>
```
...
- duplicate the full CV modal implementation for Job Match

### Step 9.5: shared UI reuse requirements

Copy Paste UI will be used across many features, so duplicated feature-local UI should be treated as a blocker for this Job Match implementation, not as a follow-up.
...
2. `CopyPasteWorkflowModal`
   - Use one shared modal for structured JSON-envelope workflows.
   ...
   - Must be used by both `CVScoreCopyPasteModal` and `JobMatchScoreCopyPasteModal` before this implementation is considered complete.
3. Feature-specific preview composition
   - Job Match likely needs a custom preview summary for `score`, `matchingKeywordsCount`, `missingKeywordsCount`, `jobKeywordsCount`, `recommendationsCount`, source, and replacement warning.
   - Pass a Job Match `renderPreview` component/function into the shared modal.
   - Do not fork the whole modal for a small preview difference.
   - If the preview is identical to another workflow, reuse the shared preview summary directly.

### Step 10: integrate modal into Job Match Analysis UI

Likely files:

```txt
src/features/job-match-analysis/components/job-match-extraction-view.tsx
src/features/job-match-analysis/components/job-match-form.tsx
src/features/job-match-analysis/components/job-match-analysis-view.tsx
src/features/job-match-analysis/hooks/use-job-match-analysis-mutations.ts
```
...
This plan applies only to Job Match Analysis. Do not change the general CV Analysis flow except to extract shared UI when duplication is proven.
...
Add feature-scoped keys under an existing Job Match Analysis namespace or a new nested namespace.
...
Find existing job match analysis scoring event stages before editing.
...
- successful apply returns `ScoreJobMatchAnalysisResponse`
...
5. Open a job match analysis without score.
...
```txt
tests/e2e/job-match-analysis-copy-paste.spec.ts
```
...
```bash
npm run test:e2e -- job-match-analysis-copy-paste
```
...
2. Job Match Analysis prompt renderer and prompt docs.
3. Job Match Analysis prepare/preview/apply use cases and tests.
4. Copy Paste API routes, validation, and responses.
5. Frontend API client and modal.
6. Job Match Analysis UI integration and translations.
