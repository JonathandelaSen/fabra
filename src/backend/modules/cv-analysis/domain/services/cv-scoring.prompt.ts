export interface CVScoringPromptInput {
  additionalContext?: string | null;
  language?: string | null;
}

export interface CVScoringCopyPastePromptInput extends CVScoringPromptInput {
  text: string;
}
...
const COPY_PASTE_WORKFLOW_ID = "cv_analysis.score";
const COPY_PASTE_SCHEMA_VERSION = "1";

export class CVScoringPrompt {
...
${this.contextBlock(input.additionalContext)}
...
Output requirements:
- ${this.languageInstruction(input.language)}
...
{
  "workflowId": "${COPY_PASTE_WORKFLOW_ID}",
  "schemaVersion": "${COPY_PASTE_SCHEMA_VERSION}",
  "result": {
    "score": <number from 0 to 100>,
    "feedback": "<Comprehensive summary of strengths and weaknesses. Be specific and actionable, in the response language defined above.>",
...
${input.text}`;
  ...
  private contextBlock(additionalContext?: string | null): string {
  ...
  private languageInstruction(language?: string | null): string {
