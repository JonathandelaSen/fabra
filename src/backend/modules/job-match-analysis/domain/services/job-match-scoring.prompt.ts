...
export interface JobMatchScoringPromptInput {
  jobDescription: string;
  jobUrl?: string | null;
  language?: string | null;
}

export interface JobMatchScoringCopyPastePromptInput
  extends JobMatchScoringPromptInput {
  text: string;
}
...
export class JobMatchScoringPrompt {
...
Output requirements:
- ${this.languageInstruction(input.language)} Keep extracted job-posting fields ("requirements", "responsibilities", "benefits", "title", etc.) faithful to the posting's own wording and language.
...
${input.text}`;
  ...
  private urlBlock(jobUrl?: string | null): string {
    return jobUrl?.trim()
