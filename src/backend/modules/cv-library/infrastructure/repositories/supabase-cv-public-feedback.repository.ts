...
type Row = {
  ...
  giver_context: string | null;
  feedback_text: string;
  created_at: string;
};
const map = (row: Row) =>
  CVPublicFeedback.fromPrimitives({
    id: row.id,
    cvId: row.cv_id,
    userId: row.user_id,
    giverName: row.giver_name,
    giverContext: row.giver_context,
    feedbackText: row.feedback_text,
    createdAt: row.created_at,
  });
export class SupabaseCVPublicFeedbackRepository
  extends BoundSupabaseRepository
  implements CVPublicFeedbackRepository
{
  async listForOwner(cvId: CVDocumentId, userId: UserId) {
    const { data, error } = await this.client
      .from("cv_public_feedback")
      .select("*")
