...
describe("SupabaseCVPublicFeedbackRepository", () => {
  it("lists and deletes owner feedback", async () => {
    ...
    const user = await createTestUser("public-cv-feedback");
    ...
    const repo = new SupabaseCVPublicFeedbackRepository();
    ...
    await repo.deleteForOwner(
      CVPublicFeedbackId.fromPrimitives(data.id),
      userId,
    );
