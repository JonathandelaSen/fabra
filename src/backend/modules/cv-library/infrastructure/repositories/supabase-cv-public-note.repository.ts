...
export class SupabaseCVPublicNoteRepository extends BoundSupabaseRepository implements CVPublicNoteRepository {
  async listForOwner(cvId: CVDocumentId, userId: UserId) {
    const { data, error } = await this.client.from("cv_public_notes").select("*").eq("cv_id", cvId.toPrimitives()).eq("user_id", userId.toPrimitives()).order("created_at");
    if (error) throw error;
    return (data as Row[]).map(map);
  }
  async listForPublishedCV(cvId: CVDocumentId) {
    const { data, error } = await this.client.from("cv_public_notes").select("*").eq("cv_id", cvId.toPrimitives()).order("created_at");
    if (error) throw error;
    return (data as Row[]).map(map);
  }
  async replaceForOwner({ cvId, userId, notes }: Parameters<CVPublicNoteRepository["replaceForOwner"]>[0]) {
    ...
    const { error: deleteError } = await this.client.from("cv_public_notes").delete().eq("cv_id", cvIdValue).eq("user_id", userIdValue);
    if (deleteError) throw deleteError;
    if (notes.length) {
      const { error } = await this.client.from("cv_public_notes").insert(notes.map((note) => { const p = note.toPrimitives(); return ({ cv_id: cvIdValue, user_id: userIdValue, anchor_type: p.anchorType, section_id: p.sectionId, anchor_id: p.anchorId, body: p.body }); }));
      if (error) throw error;
    }
