...
import type { OpportunitiesMetricsResponse } from "./responses";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await adminModule.getOpportunitiesContentMetrics.execute(parsed.value);

    return ok({
      counts: result.counts.toPrimitives(),
      windowDays: result.windowDays,
    } satisfies OpportunitiesMetricsResponse);
  } catch (error: unknown) {
